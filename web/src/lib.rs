...
use axum_login::{AuthManagerLayerBuilder, tower_sessions::{Expiry, MemoryStore, SessionManagerLayer}};
use entity_api::user::AuthBackend;
...
pub async fn init_server(app_state: AppState) -> Result<()> {
    ...
    // Auth service
    let auth_backend = AuthBackend::new(app_state.db_conn_ref().unwrap());
